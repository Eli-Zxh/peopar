...
### v3.1（进行中，代码已部分落地）
- [x] 图谱交互重做：自管视口缩放/平移/双击还原、三级文字阈值、连线高亮
- [x] vault 离线论文库（_db/papers_*.json）+ 论文页全信息（离线可读）+ 显眼实时开关
- [x] papers.keynote 一句话字段 + 多工作区（大方向）切换器 + 方向笔记连贯叙述渲染
- [ ] keynote 内容生产：新一轮 LLM 总结产出论文/研究者/方向一句话 + 方向连贯叙述（无模板腔）
- [ ] 每大方向「领域小综述」笔记（skill 产出至 peopar 上层/../，作为该方向的入口综述）
- [ ] 图谱视觉参数实机校准（缩放灵敏度/线色/文字密度）——待你重载验证反馈
...
### ⏸ v3.1 图谱前端遗留（已按用户指示暂停打磨，待回炉）
- [ ] 连线在用户端未见渲染：待确认左上角『连线 N作者·N跨向』角标数字；若数据有而未渲染→ 改 graph 系/layer 画线
- [ ] 论文 hover 一句话需 keynote（见下内容任务）；tooltip 命中与交互手感待实机回炉
- [ ] 缩放/平移手感、文字三级阈值与重叠、方向圈聚焦等——等本轮内容稳定后再统一打磨
- [ ] 研究者/论文展示、多工作区、实时开关——代码已落地，需实机回归

### ▶ 内容总结范式（本轮转向）：面向新人的连贯总结 + 一句话关键词
- [ ] keynote（20-50 字）：论文（图+被引 top 与代表）、研究者（144 图谱作者起步）、方向
- [ ] 方向/研究者「连贯叙述」重写：四字段降为审查项，正文面向对细分领域完全陌生者可读
- [ ] 每大方向「领域小综述」笔记 → 写至 <vault>/<大方向>/（peopar 上一级）
- [ ] 执行工具与批次流程（workflow/skill 会话），首批试点后铺开
## 🛠 后续增强（未实施）
